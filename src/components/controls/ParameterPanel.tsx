  ...
  return (
    <div className="flex flex-col text-sm h-full">
      ...
      <div className="flex border-b border-border shrink-0">
        {(['generate', 'tune', 'export'] as const).map((t) => (
          <button
            key={t}
            type="button"
            onClick={() => setTab(t)}
            className={cn(
              'flex-1 h-10 text-xs font-medium capitalize transition-colors',
              tab === t
...
function TuneTab() {
  ...
  const activeMode = getModeDefinition(params.modeId)
  ...
  return (
    <>
      ...
      <Section title={activeMode?.name ?? 'Mode'}>
  ...
  return (
    <div className="p-3 flex flex-col gap-3">
      <p className="text-xs text-sidebar-muted">
        Use the Export button in the toolbar to download your logo as SVG or PNG.
      </p>
      <SavedVariationsRail />
    </div>
  )
